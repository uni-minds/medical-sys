...
const GroupTypePacsStudiesId = "pacs_studies_id"
const GroupTypeVideoMid = "video_mid"
...
type GroupInfo struct {
	...
	MediaVideoMids  string `gorose:"media_video_mids"`
	MediaStudiesIDs string `gorose:"media_studies_ids"`
...
func initGroupDB() {
	...
	"media_video_mids" TEXT default "[]" not null,
	"media_studies_ids" TEXT default "[]" not null,
	"users" TEXT default "{}" not null,
	"memo" TEXT default "" not null,
	"media_counts" INTEGER not null default 0)`, global.DefaultDatabaseGroupTable)
...
func GroupGetMedia(gid int) (mids []int, err error) {
	...
	err = json.Unmarshal([]byte(gl.MediaVideoMids), &mids)
	return
}
...
func groupUpdateMedia(gid int, mids []int) error {
	...
	data := map[string]interface{}{"media_video_mids": j}
	return groupUpdate(gid, data)
}
