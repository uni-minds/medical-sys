...
func (pusher *Pusher) Logger() *logger.Logger {
	if pusher.Session != nil {
...
func NewClientPusher(client *RTSPClient) (pusher *Pusher) {
	pusher = &Pusher{
		...
		gopCacheEnable: global.GetRtspSettings().GopCacheEnable,
...
func (pusher *Pusher) bindSession(session *Session) {
	...
	session.RTPHandles = append(session.RTPHandles, func(pack *RTPPack) {
		if session != pusher.Session {
			pusher.Logger().Printf("Session recv rtp to pusher.but pusher got a new session[%v].", pusher.Session.ID)
			return
		}
		pusher.QueueRTP(pack)
	})
	session.StopHandles = append(session.StopHandles, func() {
		if session != pusher.Session {
			pusher.Logger().Printf("Session stop to release pusher.but pusher got a new session[%v].", pusher.Session.ID)
			return
		}
		pusher.ClearPlayer()
		pusher.Server().RemovePusher(pusher)
...
func (pusher *Pusher) Start() {
	for !pusher.Stoped() {
		...
		if pack == nil {
			if !pusher.Stoped() {
				pusher.Logger().Println("pusher not stoped, but queue take out nil pack")
			}
			continue
		}
...
func (pusher *Pusher) AddPlayer(player *Player) *Pusher {
	...
	if _, ok := pusher.players[player.ID]; !ok {
		...
		go player.Start()
		pusher.Logger().Printf("%v start, now player size[%d]", player, len(pusher.players))
	}
	pusher.playersLock.Unlock()
	return pusher
}
...
func (pusher *Pusher) RemovePlayer(player *Player) *Pusher {
	...
	pusher.Logger().Printf("%v end, now player size[%d]", player, len(pusher.players))
	pusher.playersLock.Unlock()
	return pusher
}
