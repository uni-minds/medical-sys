...
func NewSession(server *Server, conn net.Conn) *Session {
	...
	timeoutMillis := global.GetRtspSettings().Timeout
	timeoutTCPConn := &RichConn{conn, time.Duration(timeoutMillis) * time.Millisecond}
	authorizationEnable := global.GetRtspSettings().AuthorizationEnable
	closeOld := global.GetRtspSettings().CloseOld
	debugLogEnable := global.GetRtspSettings().AuthorizationEnable
	sessionId := fmt.Sprintf("RS_%s", tools.RandString0f(3))
	session := &Session{
		ID:                  sessionId,
		Server:              server,
		Conn:                timeoutTCPConn,
		...
		StartAt:             time.Now(),
		Timeout:             global.GetRtspSettings().Timeout,
	...
	session.Logger = logger.NewLogger(sessionId)
	return session
}
...
func (session *Session) Start() {
	...
	for !session.Stoped {
		...
		if buf1[0] == 0x24 { //rtp data
			...
			default:
				logger.Println("unknow rtp pack type, %v", channel)
				continue
			}
					...
					if len(line) == 0 {
						...
						if contentLen > 0 {
							...
							if n, err := io.ReadFull(session.connRW, bodyBuf); err != nil {
								...
								logger.Println("read rtsp request body failed, expect size[%d], got size[%d]", contentLen, n)
								return
							}
...
func CheckAuth(authLine string, method string, sessionNonce string) error {
	...
	if global.GetRtspSettings().ClientUser != username {
		return fmt.Errorf("CheckAuth error : user not exists")
	}
	md5UserRealmPwd := fmt.Sprintf("%x", md5.Sum([]byte(fmt.Sprintf("%s:%s:%s", username, realm, global.GetRtspSettings().ClientPassword))))
...
func (session *Session) handleRequest(req *Request) {
	...
	if req.Method != "OPTIONS" {
		if session.authorizationEnable {
			...
			if authFailed {
				...
				nonce := fmt.Sprintf("%x", md5.Sum([]byte(tools.RandString0f(16))))
				session.nonce = nonce
	...
	case "ANNOUNCE":
		...
		if ok {
			...
			logger.Printf("audio codec[%s]", session.ACodec)
		}
		sdp, ok = session.SDPMap["video"]
		if ok {
			session.VControl = sdp.Control
			session.VCodec = sdp.Codec
			logger.Printf("video codec[%s]", session.VCodec)
		}
		addPusher := false
		if session.closeOld {
