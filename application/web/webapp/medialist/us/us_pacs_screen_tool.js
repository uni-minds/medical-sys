...
class ScreenPlot {
    ...
    constructor(parent) {
        ...
        this.readonly = u['readonly'] !== "false"
        ...
        let btnMemo = $(`<li class="nav-item" style="margin-left:20px"><a class="nav-link active" >备注</a></li>`).click(() => {
            OpSeriesMemo(this.studies_id, this.series_id)
        })
    ...
    load() {
        $.get(`/api/v1/screen/studies/${this.studies_id}/series/${this.series_id}/details?src=ui`, result => {
            if (result.code === 200) {
...
class Communicator {
    ...
    submitValue(instance_id, selector, value) {
        ...
        let u = `/api/v1/screen/studies/${this.studies_id}/series/${this.series_id}/instances/${instance_id}/submit`+ (this.review?"?action=review":"")
    ...
    submitProgress() {
        ...
        OpAuthorSubmit(this.studies_id, this.series_id)
    }
}

function common_memo() {
    console.log("memo")
}
